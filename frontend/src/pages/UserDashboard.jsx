...
import { Terminal, PlusCircle, Ticket as TicketIcon, LogOut, Sparkles, ArrowRight, CheckCircle2 } from 'lucide-react';
import { fetchNlpSuggestions, sendNlpFeedback } from '../api/nlp';

export default function UserDashboard({ tickets, setTickets, onLogout }) {
  const [currentView, setCurrentView] = useState('new-ticket');
  const [formData, setFormData] = useState({ issue: '', category: 'Software', description: '' });
  ...
  const handleSubmit = (e, status = 'Open', autoRes = '') => {
    ...
    const newTicket = {
      ...
      ai_suggestion: nlpSuggestions.length > 0 ? nlpSuggestions[0].ai_suggestion : 'Pending review.',
      resolution: autoRes,
      author: 'me',
    };
  ...
  const handleUseSuggestion = async (suggestion) => {
    ...
    handleSubmit(null, 'Resolved', `Self-resolved via NLP: ${suggestion.ai_suggestion}`);
  };
          ...
          ticketSubmitted ? (
            <div className="h-full flex items-center justify-center p-8 animate-in fade-in duration-500">
              <div className="max-w-md w-full bg-[#111115] p-8 rounded-lg border border-zinc-800/60 text-center">
                ...
                <button onClick={() => { setTicketSubmitted(false); setFormData({ issue: '', category: 'Software', description: '' }); }} className="px-6 py-2 mt-4 bg-white text-black font-semibold rounded-md hover:bg-zinc-200 text-sm">Submit Another</button>
              </div>
            </div>
          ) : (
          ...
          <div className="p-8 h-full flex flex-col animate-in fade-in duration-500">
            ...
            <div className="bg-[#111115] rounded-lg border border-zinc-800 flex-1 overflow-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-[#0B0B0F]"><tr><th className="p-4 text-xs text-zinc-500">ID</th><th className="p-4 text-xs text-zinc-500">Issue</th><th className="p-4 text-xs text-zinc-500">Status</th></tr></thead>
